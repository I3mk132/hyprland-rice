...
def fetch_data(config):
    # The original way: just fetching the timing data for the display
...
def validate_location(city, country):
    """Uses OpenStreetMap to verify if the city and country actually exist."""
    print(f"\n{FG}Searching global maps for '{city}, {country}'...{RESET}")
    # Nominatim API is free and requires no key
    url = f"https://nominatim.openstreetmap.org/search?city={city}&country={country}&format=json"
    # OpenStreetMap requires a User-Agent header to know who is making the request
    headers = {"User-Agent": "SalahWaybarWidget/1.0"}
    try:
        response = requests.get(url, headers=headers)
        data = response.json()
        
        # If the list is greater than 0, OpenStreetMap found the location
        if len(data) > 0:
            return True
        return False
    except Exception:
        # If your internet drops during validation, it defaults to True so you aren't locked out
        return True
...
def main():
    while True:
        ...
        print(f"[{YELLOW}1{RESET}] Change Location")
        ...
        if choice == '1':
            ...
            # Asks for Country first, then City as requested
            ...
            if new_city and new_country:
                if validate_location(new_city, new_country):
                    ...
                    print(f"{RED}Error: OpenStreetMap could not find '{new_city}' in '{new_country}'.{RESET}")
