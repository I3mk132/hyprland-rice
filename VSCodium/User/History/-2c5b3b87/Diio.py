...
def solve_captcha(driver):
    try:
        ...
        # 4. فتح الصورة الكاملة وقص الكابتشا منها
        img = Image.open("full_page.png")
        captcha_img = img.crop((left, top, right, bottom))
        
        # --- 🌟 مرحلة تحسين وتنظيف الصورة 🌟 ---
        # أ) تحويل الصورة للأبيض والأسود (Grayscale) لإزالة الألوان الجانبية
        captcha_img = captcha_img.convert('L')
        
        # ب) تحويلها إلى صورة ثنائية (Black & White) عبر حد عتبة (Threshold)
        # هذا السطر يمسح أي تشويش رمادي خلف الأرقام ويجعل الرقم أسود حاد خلفية بيضاء
        captcha_img = captcha_img.point(lambda x: 0 if x < 120 else 255, '1')
        
        # حفظ الصورة المنظفة لرؤيتها والتأكد منها
        captcha_img.save("captcha.png") 
        print("[+] تم تنظيف الكابتشا وحفظها بنجاح كـ captcha.png")
        
        # 5. القراءة عبر Tesseract مع تحديد لغة الأرقام فقط وتدقيق السطر
        import pytesseract
        # أضفنا إعداد يخبر التيسراكت بأن يبحث فقط عن أرقام وعلامة الجمع والطرح والمساواة
        custom_config = r'--psm 6 -c tessedit_char_whitelist=0123456789+-='
        text = pytesseract.image_to_string(captcha_img, config=custom_config)
        print(f"[+] النص المستخرج بعد التنظيف: '{text.strip()}'")
