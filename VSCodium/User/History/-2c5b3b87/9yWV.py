...
def check_obs():
    ...
    # options.add_argument("--headless")  # إلغاء التعليق هنا لتشغيله بالمخفي لاحقاً في الخلفية
    ...
    try:
        ...
        time.sleep(5) # وقت كافٍ لتخطي الدخول وتحميل الصفحة الرئيسية
        
        # --- 🌟 الانتقال لقسم العلامات في القائمة الجانبية 🌟 ---
        print("[*] محاولة الانتقال إلى قائمة الدرجات...")
        try:
            # بناءً على صور السيستم، الزر يسمى 'Özet Not Durumu' أو يمكنك كتابة 'Not Listesi' لو اختلف
            not_listesi_btn = driver.find_element(By.XPATH, "//span[contains(text(), 'Özet Not Durumu') or contains(text(), 'Not Listesi')]")
            not_listesi_btn.click()
            print("[+] تم الضغط على قائمة الدرجات بنجاح.")
            time.sleep(4) # انتظر حتى يتم جلب البيانات داخل الـ iframe
        except Exception as menu_error:
            print(f"[-] لم يتمكن السكربت من الضغط على زر القائمة الجانبية: {menu_error}")
            print("[*] سنحاول الدخول للـ iframe مباشرة كخيار بديل...")
        ...
        try:
            ...
            # 3. منطق المقارنة والإشعار لسطح المكتب
            ...
            print(f"[-] لم يتم العثور على جدول الدرجات (grdStyle). تأكد من نجاح الدخول وصحة القائمة: {table_error}")
