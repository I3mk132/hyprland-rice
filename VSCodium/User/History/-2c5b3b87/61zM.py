...
def solve_captcha_with_gemini():
    try:
        # استدعاء عميل جيميناي باستخدام المفتاح
...
def check_obs():
    ...
    # options.add_argument("--headless")  # إلغاء التعليق هنا لاحقاً لتشغيله بالمخفي في الخلفية
    ...
    try:
        ...
        print("[*] تم ضغط زر الدخول، في انتظار تحميل الصفحة الرئيسية...")
        time.sleep(6) # وقت كافٍ لتخطي الدخول وتحميل التبويب الافتراضي تلقائياً
        
        # 1. الانتقال إلى الـ iframe برمجياً لرؤية الجدول الداخلي
        print("[*] محاولة الغوص داخل الـ iframe...")
        driver.switch_to.frame("IFRAME1")
        time.sleep(2)
        
        # 2. استخراج نص الجدول باستخدام الـ ID الصريح والثابت المكتشف بالصورة
        try:
            grades_table = driver.find_element(By.ID, "grd_not_listesi")
            ...
            print(f"[-] لم يتم العثور على جدول الدرجات (grd_not_listesi): {table_error}")
            
    except Exception as e:
        print(f"[-] حدث خطأ غير متوقع في السكربت: {e}")
    finally:
        driver.quit()
        # تنظيف الصور المؤقتة فوراً من المجلد
